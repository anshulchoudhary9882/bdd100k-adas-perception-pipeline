...
#BASE_DIR = Path(os.getenv("APP_BASE_DIR", ".")).parent
BASE_DIR = Path(os.path.dirname(os.path.abspath(__file__))).parent

TRAIN_IMG_DIR = (
    BASE_DIR / "data" / "bdd100k_images_100k" / "bdd100k" / "images" / "100k" / "train"
)
VAL_IMG_DIR = (
    BASE_DIR / "data" / "bdd100k_images_100k" / "bdd100k" / "images" / "100k" / "val"
)
...
def load_report(filepath: str) -> Optional[Dict[str, Any]]:
    """Load a JSON analytics report from disk.

    Parameters:
        filepath: Path to the JSON report file.

    Returns:
        Parsed report dictionary if the file exists and is valid JSON,
        otherwise None if the file cannot be found.
    """
    try:
...
def render_image_with_boxes(
    ...
    """Load an image from the dataset and overlay bounding boxes and labels.

    Parameters:
        image_name: Filename of the frame to load.
        boxes: List of bounding box coordinates in [x1, y1, x2, y2] format.
        categories: List of class labels corresponding to each bounding box.
        overlay_text: Optional annotation text to display on the image.

    Returns:
        RGB image array with drawn boxes and overlay text. If the image cannot be
        found locally, returns a placeholder image explaining the missing file.
    """
    ...
    for box, category in zip(boxes, categories):
        ...
        (text_w, text_h), _ = cv2.getTextSize(
            category, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 1
        )
...
selected_weather = st.sidebar.multiselect(
    "weather", available_weather, default=available_weather
)
selected_time = st.sidebar.multiselect(
    "Time of Day", available_time, default=available_time
)


def get_class_df(data: Dict[str, Any]) -> pd.DataFrame:
    """Transform class distribution report data into a display-ready DataFrame.

    Parameters:
        data: Analytics report dictionary containing class distribution counts.

    Returns:
        DataFrame with class names, instance counts, image counts, and computed
        instances-per-image ratios sorted by descending instance count.
    """
    dist = data.get("class_distribution", {})
    df = pd.DataFrame.from_dict(dist, orient="index").reset_index()
    df.rename(
        columns={"index": "Class", "count": "Instances", "images": "Images"},
        inplace=True,
    )
...
if selection == "Overview & Metadata":
    ...
    with col_pareto:
        ...
        df_classes["cumulative_pct"] = (
            df_classes["Instances"].cumsum() / df_classes["Instances"].sum() * 100
        )
        ...
        fig_pareto.add_trace(
            go.Bar(
                x=df_classes["Class"],
                y=df_classes["Instances"],
                name="Annotations",
                marker_color="#3b82f6",
                text=df_classes["Instances"],
    ...
    st.dataframe(
        df_classes.style.background_gradient(
            cmap="Blues", subset=["Instances", "Images", "Instances per Image"]
        ),
        use_container_width=True,
    )
    st.markdown("---")
    ...
    if geo_samples:
        ...
        st.caption(
            "Displays the 2D histogram of object centers. Useful for ADAS ground-plane calibration checks."
        )
        if "center_x" in df_geo.columns and "center_y" in df_geo.columns:
            fig_spatial = px.density_heatmap(
                df_geo,
                x="center_x",
                y="center_y",
                facet_col="category",
    ...
    if not val_data:
        ...
        train_cls.rename(
            columns={"Class": "Category", "Instances": "Count"}, inplace=True
        )
        ...
        val_cls["Percentage"] = (val_cls["Count"] / val_cls["Count"].sum() * 100).round(
            2
        )
        ...
        sort_order = train_cls.sort_values("Percentage", ascending=False)[
            "Category"
        ].tolist()
        ...
        fig_cls.update_traces(texttemplate="%{text:.2f}%", textposition="outside")

        fig_cls.update_layout(template="plotly_dark", yaxis_title="Percentage (%)")
        ...
        # ============================================================
        train_w = train_data.get("metadata_distribution", {}).get("weather", {})

        val_w = val_data.get("metadata_distribution", {}).get("weather", {})

        df_w_train = pd.DataFrame(list(train_w.items()), columns=["weather", "Count"])

        df_w_val = pd.DataFrame(list(val_w.items()), columns=["weather", "Count"])

        df_w_train["Percentage"] = (
            df_w_train["Count"] / df_w_train["Count"].sum() * 100
        ).round(2)

        df_w_val["Percentage"] = (
            df_w_val["Count"] / df_w_val["Count"].sum() * 100
        ).round(2)
        ...
        sort_order_weather = df_w_train.sort_values("Percentage", ascending=False)[
            "weather"
        ].tolist()
        ...
        fig_weather.update_traces(texttemplate="%{text:.2f}%", textposition="outside")

        fig_weather.update_layout(template="plotly_dark", yaxis_title="Percentage (%)")
        ...
        # scene DISTRIBUTION (%)
        # ============================================================
        train_s = train_data.get("metadata_distribution", {}).get("scene", {})

        val_s = val_data.get("metadata_distribution", {}).get("scene", {})

        df_s_train = pd.DataFrame(list(train_s.items()), columns=["scene", "Count"])

        df_s_val = pd.DataFrame(list(val_s.items()), columns=["scene", "Count"])

        df_s_train["Percentage"] = (
            df_s_train["Count"] / df_s_train["Count"].sum() * 100
        ).round(2)

        df_s_val["Percentage"] = (
            df_s_val["Count"] / df_s_val["Count"].sum() * 100
        ).round(2)
        ...
        sort_order_scene = df_s_train.sort_values("Percentage", ascending=False)[
            "scene"
        ].tolist()
        ...
        fig_scene.update_traces(texttemplate="%{text:.2f}%", textposition="outside")

        fig_scene.update_layout(template="plotly_dark", yaxis_title="Percentage (%)")
        ...
        df_merged["pct_train"] = df_merged["Percentage_train"] / 100

        df_merged["pct_val"] = df_merged["Percentage_val"] / 100

        kl_div = entropy(df_merged["pct_train"], df_merged["pct_val"])
        ...
        df_merged["Drift (%)"] = (
            df_merged["Percentage_val"] - df_merged["Percentage_train"]
        ).round(2)

        st.dataframe(
            df_merged.sort_values(by="Drift (%)", key=abs, ascending=False),
            use_container_width=True,
        )
    ...
    st.write(
        "Analyzing object dimensions to detect potential labeling errors or physical limitations for camera sensors. Data represents a 5,000-point random subsample."
    )
    ...
    with colB:
        ...
        st.metric(
            "Train Split Extreme Aspects", f"{train_anom.get('extreme_aspects', 0):,}"
        )
    ...
    if geom_samples:
        ...
        fig_geom = px.scatter(
            ...
            labels={
                ...
                "area_ratio": "Area Ratio (% of Image)",
            },
        )
        fig_geom.add_vline(
            x=5.0,
            line_dash="dash",
            line_color="red",
            annotation_text="Extreme Width Anomaly (>5.0)",
        ...
        fig_geom.add_vline(
            ...
            line_dash="dash",
            line_color="blue",
            annotation_text="Extreme Height Anomaly (<0.2)",
            annotation_position="top right",
        )
        fig_geom.add_hline(
            y=0.001,
            line_dash="dash",
            line_color="orange",
            annotation_text="Micro-Box Anomaly (<0.1% Area)",
    ...
    with tab1:
        ...
        sample_type = st.selectbox(
            "Select Sample Type",
            ["largest_objects", "smallest_objects", "high_density_frames"],
        )
        ...
        if data_list:
            ...
            selected_row = st.selectbox(
                "Select a frame to visualize:", df_samples["name"].unique()
            )
            ...
            if sample_type == "largest_objects":
                ...
                overlay = (
                    f"LARGEST {cat} | Area: {row_data.get('area_ratio', 0) * 100:.1f}%"
                )
            elif sample_type == "smallest_objects":
                cat = row_data.get("category", "Unknown").upper()
                overlay = (
                    f"SMALLEST {cat} | Area: {row_data.get('area_ratio', 0) * 100:.3f}%"
                )
            else:
            ...
            if st.button("Load Edge Case Image"):
                img = render_image_with_boxes(
                    selected_row, boxes, labels, overlay_text=overlay
                )
                st.image(
                    img, caption=f"Frame: {selected_row}", use_container_width=True
                )

    with tab2:
        st.caption(
            "Filter and identify frames where the environment is complex and model confidence might drop."
        )
        ...
        if hard_samples:
            ...
            df_filtered = df_hard[
                (df_hard["weather"].isin(selected_weather))
                & (df_hard["timeofday"].isin(selected_time))
            ]

            col1, col2 = st.columns(2)
            max_score = (
                float(df_filtered["score"].max()) if not df_filtered.empty else 10.0
            )
            min_hardness = col1.slider(
                "Minimum Hardness Score",
                min_value=0.0,
                max_value=max_score,
                value=min(2.0, max_score),
            )
            col2.slider(
                "Max Model Confidence (Mocked feature for Active Learning)",
                min_value=0.0,
                max_value=1.0,
                value=0.4,
            )
            ...
            st.dataframe(
                df_filtered.style.background_gradient(cmap="Reds", subset=["score"]),
                use_container_width=True,
                height=250,
            )
            ...
            if not df_filtered.empty:
                ...
                hard_selected = st.selectbox(
                    "Select Hard Frame:", df_filtered["name"].unique()
                )
                ...
                if st.button("Load Hard Frame"):
                    ...
                    img = render_image_with_boxes(
                        hard_selected, [], [], overlay_text=overlay
                    )
                    st.image(
                        img, caption=f"Frame: {hard_selected}", use_container_width=True
                    )
